...
Best-effort Step-7 phase recovery. For now this verifies the zero phase lift and
warns otherwise, returning `phase_verified=false` instead of pretending the
missing SympleQ companion-paper rules are known.
"""
function recover_phase_vector(tab::SymplecticTableau, perm::TermPermutation, S::SymplecticMatrix)
    ...
    if !verified
        @warn(
            "SympleQ phase recovery is unverified for this generator; returning the binary symplectic action with phase_verified=false."
        )
    end
