...
# Mosek solve on the selected refinement.
#
# Examples:
#   julia --project demos/h4_periodic_native_v2rdm_mosek_benchmark.jl
#   julia --project demos/h4_periodic_native_v2rdm_mosek_benchmark.jl --refinement=spin_resolved --time-limit=60
...
function parse_flags(args)
    ...
    return (; refinement, time_limit, include_traces, num_threads, log_level)
end
...
println("H4 periodic Nk=2 — D-only bridge / native V2RDM Mosek benchmark")
println("selected refinement: ", refinement_label(CLI.refinement))
println(@sprintf("Mosek config: time_limit = %.1f s, num_threads = %d, log_level = %d",
...
println("\n[stage] optimize! (Mosek) : running native JuMP model ...")
