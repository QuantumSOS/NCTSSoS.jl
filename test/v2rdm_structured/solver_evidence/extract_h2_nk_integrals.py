#!/usr/bin/env python3
"""Extract physical periodic H2 active-space integrals for arbitrary Nk.

This is the PySCF side of the H2/Nk sweep.  It uses the Figure-1-matching
1D-chain setup used by the earlier Nk=2 run:

  * H2 unit cell at x = 0,1 Å, lattice a = 2 Å
  ...
  * first 4 spatial MOs per k-point as the active space

Outputs are intentionally boring:

  <outdir>/h2_chain_nk<N>_integrals_drop<TOL>.txt
      Unnormalized h1e/eri text dump consumed by the Julia NCTSSoS exporter.
  <outdir>/h2_chain_nk<N>.npz
      Full NumPy archive for audit/reuse.
  <outdir>/h2_chain_nk<N>_meta.json
...
H  1.0  0.0  0.0
"""
    cell.a = [[2.0, 0.0, 0.0], [0.0, args.vacuum, 0.0], [0.0, 0.0, args.vacuum]]
...
def write_text_integrals(path: Path, h1e: dict[int, np.ndarray], eri: dict[tuple[int, int, int, int], np.ndarray], meta: dict[str, Any], drop_tol: float) -> tuple[int, int]:
    ...
    with path.open("w", encoding="utf-8") as f:
        f.write("# Physical periodic H2 active-space integrals from PySCF\n")
...
def extract(args: argparse.Namespace) -> None:
    ...
    print("== physical H2 periodic integral extraction ==")
    print(
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument("--n-active-orb", type=int, default=4)
    p.add_argument("--n-active-elec", type=int, default=2)
