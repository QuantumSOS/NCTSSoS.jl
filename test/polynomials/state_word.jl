...
@testset "StateWord{MaxEntangled} (Tracial)" begin
    ...
    @testset "Cyclic Canonicalization for Trace" begin
        ...
        m_abc = NormalMonomial{NonCommutativeAlgebra}([1, 2, 3])
        m_bca = NormalMonomial{NonCommutativeAlgebra}([2, 3, 1])
        m_cab = NormalMonomial{NonCommutativeAlgebra}([3, 1, 2])
        ...
        m_cba = NormalMonomial{NonCommutativeAlgebra}([3, 2, 1])
        sw_cba = tr(m_cba)
        @test sw_abc == sw_cba

        # Test with longer words
        m_12345 = NormalMonomial{NonCommutativeAlgebra}([1, 2, 3, 4, 5])
        m_23451 = NormalMonomial{NonCommutativeAlgebra}([2, 3, 4, 5, 1])
        m_34512 = NormalMonomial{NonCommutativeAlgebra}([3, 4, 5, 1, 2])
...
@testset "StateWord{Arbitrary}" begin
    ...
    @testset "Involution (not Cyclic) Canonicalization for Arbitrary" begin
        ...
        m_abc_adj = NormalMonomial{NonCommutativeAlgebra}([-3, -2, -1])
...
@testset "NCStateWord{MaxEntangled}" begin
    ...
    @testset "Degree" begin
        ...
        m_nc = NormalMonomial{NonCommutativeAlgebra}([3, 4])      # degree 2
    ...
    @testset "Adjoint" begin
        m_state = NormalMonomial{NonCommutativeAlgebra}(UInt8[1, 2])
        m_nc = NormalMonomial{NonCommutativeAlgebra}(UInt8[3, 4])
...
@testset "NCStateWord{Arbitrary}" begin
    ...
    @testset "Hash and Uniqueness" begin
        m = NormalMonomial{NonCommutativeAlgebra}([1, 2])
        sw = ς(m)
        nc_word = NormalMonomial{NonCommutativeAlgebra}([3])
...
@testset "StateSymbol Canonicalization" begin
    ...
    @testset "StateSymbol{MaxEntangled} - Cyclic Symmetric Canon" begin
        ...
        m_bca = NormalMonomial{NonCommutativeAlgebra}([2, 3, 1])
        m_cab = NormalMonomial{NonCommutativeAlgebra}([3, 1, 2])
        m_cba = NormalMonomial{NonCommutativeAlgebra}([3, 2, 1])  # reverse (adjoint for NC)
    ...
    @testset "StateSymbol - Basic Properties" begin
        ...
        # isless (degree-first)
        m_longer = NormalMonomial{NonCommutativeAlgebra}([1, 2, 3])
