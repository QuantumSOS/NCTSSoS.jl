...
@assert length(eri) == cs_only["eri_blocks"] #src
nothing #hide
...
@assert length(h.terms) == cs_only["hamiltonian_terms"] #src
...
@assert length(sparsity.corr_sparsity.cliques) == cs_only["correlative_cliques"] #src
@assert maximum(clique_sizes) == cs_only["max_clique_size"] #src
@assert moment_basis_sizes == [cs_only["order2_basis_size"]] #src
...
@assert moment_problem.n_unique_moment_matrix_elements == cs_only["order2_nuniq"] #src
@assert length(moment_problem.constraints) == cs_only["moment_problem_constraints"] #src
