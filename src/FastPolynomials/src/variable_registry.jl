...
"""
    monomial_ring(::Type{A}, prefix_subscripts) where A<:AlgebraType

Create a registry and grouped monomial vectors for the given algebra type.

This function provides a convenient way to create both a variable registry
and the corresponding monomials in a single call. Each prefix group becomes
a separate vector of monomials with the appropriate algebra type.

**Note:** This is equivalent to calling `create_projector_variables`,
`create_unipotent_variables`, or `create_noncommutative_variables` directly.
Use this when you need to specify a custom algebra type.

# Arguments
- `A`: The algebra type (e.g., `ProjectorAlgebra`, `UnipotentAlgebra`, `NonCommutativeAlgebra`)
- `prefix_subscripts`: Vector of `(prefix, subscripts)` tuples for multi-prefix creation

# Returns
A tuple of:
- `registry::VariableRegistry`: Maps symbols to encoded indices
- `monomial_groups::Tuple`: Tuple of `Vector{Monomial{A,T}}` for each prefix group

# Examples
```jldoctest
julia> reg, (P, Q) = monomial_ring(ProjectorAlgebra, [("P", 1:3), ("Q", 4:5)]);

julia> length(P)
3

julia> P[1] isa Monomial{ProjectorAlgebra}
true
```

Single group example:
```jldoctest
julia> reg, (x,) = monomial_ring(NonCommutativeAlgebra, [("x", 1:5)]);

julia> length(x)
5
```
"""
function monomial_ring(
    ::Type{A},
    prefix_subscripts::Vector{Tuple{String, VT}}
) where {A<:AlgebraType, T<:Integer, VT<:AbstractVector{T}}
    _create_noncommutative_variables(A, prefix_subscripts)
end
