...
function prepare()
    ...
    return basis, one_poly, row_bases, reducer, MP
end

function main()
    block_idx = parse(Int, get(ENV, "BLOCK_INDEX", "14"))
    max_entries = parse(Int, get(ENV, "MAX_ENTRIES", "800"))
    profile_path = get(ENV, "PROFILE_OUT", "scratch/order3_direct_entries_profile.txt")
    profile_flat_path = replace(profile_path, ".txt" => "_flat.txt")

    stage("prepare")
    basis, one_poly, row_bases, reducer, MP = prepare()
    sizes = [size(B, 1) for B in row_bases]
    println("block sizes = ", sizes)
    B = row_bases[block_idx]
    row_indices, row_values = NCTSSoS._sparse_row_supports(B)
    n = size(B, 1)
    entries = Tuple{Int,Int}[]
    sizehint!(entries, max_entries)
    for j in 1:n, i in 1:j
        push!(entries, (i, j))
        length(entries) >= max_entries && break
    end
    support_pairs = sum(length(row_indices[i]) * length(row_indices[j]) for (i, j) in entries)
    println("profile block = ", block_idx, " size=", n, " nnz=", nnz(B), " entries=", length(entries), " support_pairs=", support_pairs)
    flush(stdout)
    ...
    elapsed = @elapsed begin
        @profile begin
            for (i, j) in entries
                p = NCTSSoS._transformed_entry_from_sparse_supports(
                    one_poly,
                    basis,
                    row_indices[i],
                    row_values[i],
                ...
                result_terms += length(p.terms)
            end
        end
    end
    stage("profiled entries done elapsed=$elapsed result_terms=$result_terms reducer_cache=$(length(reducer.reduction))")
